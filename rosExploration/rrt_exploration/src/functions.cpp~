...
// rdm class, for gentaring random flot numbers     生成随机点
rdm::rdm() {i=time(0);}
float rdm::randomize() { i=i+1;  srand (i);  return float(rand())/float(RAND_MAX);}



//Norm function      Norm函数计算两个点之间的距离
...
//sign function       sign函数 当n小于0，返回-1，否则返回1
float sign(float n)
{
if (n<0.0){return -1.0;}
else{return 1.0;}
...
//Nearest function     nearest函数 返回最近的点
...
//Steer function      steer函数 返回新生成的随机点
...
//gridValue function    grid函数 返回地图指定位置的占据情况 100表示占据，-1表示未知，0表示空闲
...
// ObstacleFree function-------------------------------------  obstaclefree函数 通过插值的方法来判断生成的新的路径中有没有障碍物 返回0表示有，-1表示未知，1表示没有
